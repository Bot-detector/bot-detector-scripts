...
-- Step 3: Update temp_batch with existing foreign keys from (report_sighting, report_gear, report_location)
...
SELECT *
FROM report_sighting
ORDER BY report_sighting_id DESC
LIMIT 10;
...
WHERE NOT EXISTS (SELECT 1
                  FROM report_gear rg
                  WHERE 1
                    AND tb.equip_head_id = rg.equip_head_id
                    AND tb.equip_amulet_id = rg.equip_amulet_id
                    AND tb.equip_torso_id = rg.equip_torso_id
                    AND tb.equip_legs_id = rg.equip_legs_id
                    AND tb.equip_boots_id = rg.equip_boots_id
                    AND tb.equip_cape_id = rg.equip_cape_id
                    AND tb.equip_hands_id = rg.equip_hands_id
                    AND tb.equip_weapon_id = rg.equip_weapon_id
...
ORDER BY report_location_id DESC
LIMIT 10;
...
SELECT DISTINCT report_sighting_id,
                report_location_id,
                report_gear_id,
                created_at,
                timestamp,

...
def _records_to_migrate() -> TextClause:
    sql = sqla.text("""
        SELECT 
            ...
            sdv.skills,
            sdv.activities
        FROM highscore_data sdv
        join (select * from Players where label_id != 0 and id >= :player_id) pl on sdv.player_id = pl.id
        ORDER BY pl.id asc
        limit :limit;
    """)
    return sql
...
async def main():
    START_PLAYER_ID: int = 3777082
    LIMIT: int = 10_000
    PRODUCE_MESSAGES: bool = True
        ...
        try:
            ...
            while i < len(data):
                ...
                payload = ScrapedStruct(
                    metadata=MetaData(version=1, source="migration_v4"),
                    player_data=PlayerStruct(
                        id=d["player_id"],
                        name=d["player_name"],
                ...
                if payload.player_data.updated_at:
                    payload.player_data.updated_at = (
                        payload.player_data.updated_at - timedelta(days=1)
                    )

                payload = payload.model_dump()

                if not PRODUCE_MESSAGES:
                    print(payload)
                    print("*" * 10)
